...
import CardService from './CardService2';
...
const ContainerInfo = ({containerName, info}: {containerName?: string, info: MonitoringInfo}) => {
  ...
  const handleViewContainerStatusDetail = ({containerID}: {containerID: ContainerID}) => {
    console.log('View container status detail');
  }

  const handleViewAPIDetail = ({containerID}: {containerID: ContainerID}) => {
    console.log('View API detail');
  }

  const handleViewUserCapacityDetail = ({containerID}: {containerID: ContainerID}) => {
    console.log('View user capacity detail');
  }

  const handleViewCPUDetail = ({containerID}: {containerID: ContainerID}) => {
    console.log('View CPU detail');
  }

  const handleViewRAMDetail = ({containerID}: {containerID: ContainerID}) => {
    console.log('View RAM detail');
  }

  const handleViewNetworkDetail = ({containerID}: {containerID: ContainerID}) => {
    console.log('View network detail');
  }
    ...
    <>
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container spacing={0.5} margin={0.5}>
          <Grid size={{ xs: 6, md: 2 }}>
            <CardService title="Container" onViewDetail={() => handleViewContainerStatusDetail({ containerID: info.containerID })}>
              {info && <ContainerStatus containerStatus={info.container}/>}
            </CardService>
          </Grid>
          <Grid size={{ xs: 6, md: 2 }}>
            <CardService title="Endpoint API" onViewDetail={() => handleViewAPIDetail({ containerID: info.containerID })}>
              {info && <APIStatus apiStatus={info.api}/>}
            </CardService>
          </Grid>
          <Grid size={{ xs: 6, md: 2 }}>
            <CardService title="User capacity" onViewDetail={() => handleViewUserCapacityDetail({containerID: info.containerID})}>
              {info && <UserCapacity userCapacity={info.userCapacity}/>}
            </CardService>
          </Grid>
          <Grid size={{ xs: 6, md: 2 }}>
            <CardService title="CPU" onViewDetail={() => handleViewCPUDetail({containerID: info.containerID})}>
              {info && <CPU cpu={info.cpu}/>}
            </CardService>
          </Grid>
          <Grid size={{ xs: 6, md: 2 }}>
            <CardService title="RAM" onViewDetail={() => handleViewRAMDetail({containerID: info.containerID})}>
              {info && <RAM ram={info.ram}/>}
            </CardService>
          </Grid>
          <Grid size={{ xs: 6, md: 2 }}>
            <CardService title="Network" onViewDetail={() => handleViewNetworkDetail({containerID: info.containerID})}>
